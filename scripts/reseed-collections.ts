...
async function reseedCollections() {
  ...
  try {
    // 1. Clear existing collections data
    console.log("🗑️  Clearing existing collections data...");
    await db.delete(collectionProducts);
    await db.delete(collections);
    console.log("✅ Cleared collections data");
    ...
    // 3. Get existing products to assign to collections
    ...
    if (collectionProductsData.length === 0) {
      console.log("⚠️ No matching products found for collections");
      console.log("   Make sure you have products with 'heart shape' or 'flower' in their names");
      process.exit(0);
    }
    ...
    console.log("\n✨ Collections Reseeding Completed Successfully!\n");
    console.log("📊 Summary:");
